    ...
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent className="w-auto p-3" side="top" sideOffset={5}>
        <p className="text-sm text-muted-foreground mb-2">
          Pinyin audio for <span className="font-bold text-foreground text-base">{pinyin}</span> / <span className="font-bold text-pink-600 dark:text-pink-400 text-base">{zhuyin}</span> is not 100% perfect.
        </p>
        <p className="text-sm text-muted-foreground mb-3">
          Find real words using the dictionary links below.
        </p>
        <div className="flex flex-col gap-2">
          ...
          <a
            ...
            onClick={() => onOpenChange(false)}
          >
            Search for words on <span className="font-semibold">Yabla</span>
            <ExternalLink className="h-3 w-3" />
          </a>
        </div>
      </PopoverContent>
